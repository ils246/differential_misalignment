...
async def process_api_requests_from_file(
    ...
    # constants
    seconds_to_pause_after_rate_limit_error = 10
    seconds_to_sleep_each_loop = (
        0.001  # 1 ms limits max throughput to 1,000 requests per second
    )
    
    # ADDED to handle stop pause
    rate_limit_event = asyncio.Event()
    rate_limit_event.set()  # initially allow requests
    ...
    api_endpoint = "chat/completions"#api_endpoint_from_url(request_url)
    request_header = {
    ...
    with open(requests_filepath) as file:
        ...
        async with aiohttp.ClientSession() as session:  # Initialize ClientSession here
            while True:
                # ADDED Explicitly wait if in rate-limit cooldown
                ...
                seconds_since_update = current_time - status_tracker.last_update_time # ADDED
                # rimesse a 0 in modo che non parte a tutta do 
                status_tracker.available_request_capacity = min(
                    status_tracker.available_request_capacity + max_requests_per_minute * seconds_since_update / 60.0,
                    max_requests_per_minute,
                )
                status_tracker.available_token_capacity = min(
                    status_tracker.available_token_capacity + max_tokens_per_minute * seconds_since_update / 60.0,
                    max_tokens_per_minute,
                )
                status_tracker.last_update_time = current_time

                # if enough capacity available, call API
                if next_request:
                    next_request_tokens = next_request.token_consumption
                    if (
                        status_tracker.available_request_capacity >= 1
                        and status_tracker.available_token_capacity >= next_request_tokens
                    ):
                        status_tracker.available_request_capacity -= 1
                        ...
                        asyncio.create_task(
                            next_request.call_api(
                                ...
                                cooldown_duration=seconds_to_pause_after_rate_limit_error
                            )
                        )
                        next_request = None
...
class APIRequest:
    ...
    async def call_api(
        ...
        rate_limit_event: asyncio.Event,  # ADDED this
        cooldown_duration: float,  # ADDED this
    ):
        ...
        try:
            ...
            if "error" in response:
                ...
                if isinstance(detail, dict):
                    error_message = detail.get("message", "").lower()
                elif isinstance(detail, str):
                    error_message = detail.lower()
                else:
                    error_message = ""

                logging.warning(f"Request {self.task_id} failed with error: {response['detail']}")

                if "rate limit" in error_message:
                    status_tracker.time_of_last_rate_limit_error = time.time()
                    status_tracker.num_rate_limit_errors += 1
                    rate_limit_event.clear()  # Immediately stop sending further requests

                    # Explicitly wait for cooldown duration before resuming
                    logging.warning(f"Rate limit hit. Cooling down for {cooldown_duration} seconds.")
                    await asyncio.sleep(cooldown_duration)

                    # Reset capacities after cooldown:
                    status_tracker.last_update_time = time.time()
                    status_tracker.available_request_capacity = 0
                    status_tracker.available_token_capacity = 0
                    
                    
                    rate_limit_event.set()  # Allow new requests after cooldown
                    retry_queue.put_nowait(self)  # Re-enqueue this request for retry
                    return  # Exit function to avoid marking as permanent failure
